...
$peakdays = $peakdays.split(",")
foreach($peakday in $peakdays){
    if($peakDay -eq $currentDay){ $isPeakDay = $true }
}
...
Write-Log 3 "Starting WVD Tenant Hosts Scale Optimization: Current Date Time is: $CurrentDateTime and Current day: is $CurrentDay" "Info"
...
if ($hostpoolInfo.LoadBalancerType -eq "DepthFirst") {
    ...
    if ($CurrentDateTime -ge $BeginPeakDateTime -and $CurrentDateTime -le $EndPeakDateTime -and $isPeakDay -eq $true) {
        Write-output "It is in peak hours/day now"
        Write-Log 1 "It is in peak hours/day now" "Info"
